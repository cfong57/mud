...
int burn_command(string cmd)
{
  object fireobj;
  if(!cmd)
  {
    fireobj = new("/u/a/allanon/obj/fireobj.c");
    fireobj->move(environment(this_player()),1);
    fireobj->start();
    message("info","Set the room on fire.\n",this_player());
    return 1;
  }
