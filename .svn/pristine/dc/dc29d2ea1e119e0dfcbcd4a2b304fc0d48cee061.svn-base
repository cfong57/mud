...
void extra_create()
{
  ...
  set_long(
  wrap("This ranger, garbed in simple, forest-green clothing, is a young elf "+
       "female - she looks to be maybe 120 years old or so. Her jade-green eyes and "+
       "shining, pale blond hair lend an air of youthfulness, and she appears "+
  ...
  if(random(5))
   add_random_weapon("random_scimitar",3,({"right hand"}));
  else
   add_weapon("/u/a/allanon/area/caverns_of_slime/obj/scimitar.c", ({"right hand"}));

  if(random(5))
   add_random_weapon("random_scimitar",3,({"left hand"}));
  else
   add_weapon("/u/a/allanon/area/caverns_of_slime/obj/scimitar2.c", ({"left hand"}));

  if(random(2))
   add_armour("/u/a/allanon/area/caverns_of_slime/obj/murinshirt.c");

  if(random(2))
   add_armour("/u/a/allanon/area/caverns_of_slime/obj/murintights.c");

  add_armour("/u/a/allanon/area/caverns_of_slime/obj/muringloves.c");
  add_armour("/u/a/allanon/area/caverns_of_slime/obj/murinboots.c");

  set_skill("melee",75);
  set_skill("weapon parry",60);
  set_skill("slashing weapons",70);
  set_skill("swords",50);
  set_skill("twin weapons",50);
  set_skill("dodge",60);
  set_skill("block",60);
  ...
  set_inquiry(([
    "$GREET": ({"!nod $TPQN", "Hello, $TPQCN. Are you also here to assist the rangers?"}),
    "rangers" : "Protectors of the forest, we are...all flora and fauna, actually.\n",
    "assist" :
    ...
    "recover" : "I'll give a reward to someone if they can find and return the heirloom.\n",
 ...
 if(has_heart)
  {
   ...
   force_me("say This must be a fake. You'll not be needing it anymore, so I'll keep it.");
   heart->remove();
   return 1;
  }
 else
  {
   ...
   message("info", "Qialnean gives "+this_player()->query_cap_name()+" something.\n",
...
int maneuver()
{
 if(this_object()->query_battleob())
  "/cmds/skills/whirling_blades"->do_maneuver(this_object(), "with singing blades");
