...
void extra_create()
{
  ...
  "%^BOLD%^%^GREEN%^vortex%^RESET%^ is changing too constantly for you to "+
  "discern much more than a general conelike shape. Eerie green light seems "+
  "to come from the %^BOLD%^%^GREEN%^vortex%^RESET%^ at occasional intervals. "+
  "Somehow you think that this is not quite the same kind of vortex created "+
  "by Sorcerers, yet it is very similar.\n"));
    ...
    if (member_array("campfire", ids) != -1) 
      {
   	if (o != this_object()) 
   	    {
   	message("info","The %^BOLD%^%^GREEN%^vortex%^RESET%^ fades away at the "+
   	"presence of the campfire.\n",environment(this_object()));
    destruct(this_object());
   	return;	
        }
    }
...
foreach (object o in all_inventory(environment(this_object())))
  { 
    ...
    if (member_array("regen_ob", ids) != -1) 
    {
   	if (o != this_object()) 
   	  {
    message("info","The already-present %^BOLD%^%^GREEN%^vortex%^RESET%^ dims "+
    "and fades at the presence of a new %^BOLD%^%^GREEN%^vortex%^RESET%^.\n",
    environment(this_object()));
    destruct(this_object());
   	return;     
	  }
